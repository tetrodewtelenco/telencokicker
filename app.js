...
async function uploadProfilePhoto(file){
  ...
  const errBox=$("#profilePhotoError");
  ...
  try{
    if(!file.type || !file.type.startsWith("image/")) throw new Error("Kies een geldig afbeeldingsbestand.");
    if(file.size > 8*1024*1024) throw new Error("Foto is te groot. Maximum 8 MB.");

    const ext=(file.name.split(".").pop()||"jpg").toLowerCase().replace(/[^a-z0-9]/g,"") || "jpg";
    const path=`${currentProfileId}/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
    ...
    await load();
    openProfile(currentProfileId,ranked());
  }catch(err){
    console.error(err);
    if(errBox) errBox.textContent="Foto wijzigen mislukt: "+(err.message||err);
  }finally{
    if(btn){
      btn.disabled=false;
      btn.textContent="Profielfoto wijzigen";
    }
  }
}
